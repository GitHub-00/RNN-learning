...
def create_model(hps, vocab_size, num_classes):
    ...
    def _generate_param_for_item_cell(x_size, h_size, bias_size):
        '''generate parameters for pure lstm implemenation'''
    ...
    with tf.variable_scope('lstm_nn', initializer= lstm_init):
        ...
        with tf.variable_scope('inputs'):
            ix, ih, ib = _generate_param_for_item_cell(x_size=[hps.num_embedding_size, hps.num_lstm_nodes[0]],
                                                       h_size=[hps.num_lstm_nodes[0],hps.num_lstm_nodes[0]],
                                                       bias_size=[1,hps.num_lstm_nodes[0]])

        with tf.variable_scope('outputs'):
            ox, oh, ob = _generate_param_for_item_cell(x_size=[hps.num_embedding_size, hps.num_lstm_nodes[0]],
                                                       h_size=[hps.num_lstm_nodes[0],hps.num_lstm_nodes[0]],
                                                       bias_size=[1,hps.num_lstm_nodes[0]])
        with tf.variable_scope('forget'):
            fx, fh, fb = _generate_param_for_item_cell(x_size=[hps.num_embedding_size, hps.num_lstm_nodes[0]],
                                                       h_size=[hps.num_lstm_nodes[0], hps.num_lstm_nodes[0]],
                                                       bias_size=[1, hps.num_lstm_nodes[0]])
        with tf.variable_scope('memory'):
            cx, ch, cb = _generate_param_for_item_cell(x_size=[hps.num_embedding_size, hps.num_lstm_nodes[0]],
                                                       h_size=[hps.num_lstm_nodes[0], hps.num_lstm_nodes[0]],
                                                       bias_size=[1, hps.num_lstm_nodes[0]])
        ...
        for i in range(num_timesteps):
            ...
            mid_states = tf.tanh(tf.matmul(embed_input, cx)+tf.matmul(h, ch)+cb)
            state = mid_states * input_gate + state * forget_gate
